...
import { Loader2, Crown, Check, DollarSign, User } from "lucide-react";
  ...
  const handleCreatePrices = async (e: React.FormEvent) => {
    ...
    try {
      for (const [loc, prices] of [["IN_CALL", inCall], ["OUT_CALL", outCall]] as const) {
        ...
        const res = await fetch("http://localhost:3000/profile/prices", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
          throw new Error(data.message || `Failed to save ${loc} prices`);
        }
      }
      setCompleted(true);
    } catch (err: any) {
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {SERVICES.map((s) => (
                  <Badge
                    key={s}
                    variant={profile.services.includes(s) ? "default" : "secondary"}
                    className={`cursor-pointer text-[10px] transition-colors ${profile.services.includes(s) ? "gold-gradient text-primary-foreground" : "bg-secondary hover:bg-secondary/80"}`}
                    onClick={() => setProfile({ ...profile, services: toggleMulti(profile.services, s) })}
                  >
                    {s.replace("_", " ")}
                  </Badge>
                ))}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {LANGUAGES.map((l) => (
                  <Badge
                    key={l}
                    variant={profile.languages.includes(l) ? "default" : "secondary"}
                    className={`cursor-pointer text-[10px] transition-colors ${profile.languages.includes(l) ? "gold-gradient text-primary-foreground" : "bg-secondary hover:bg-secondary/80"}`}
                    onClick={() => setProfile({ ...profile, languages: toggleMulti(profile.languages, l) })}
                  >
                    {l}
                  </Badge>
                ))}
                  ...
                  {([["30 min", "price30min"], ["1 hour", "price1hour"], ["Whole Night", "priceWholeNight"]] as const).map(([lbl, key]) => (
                    <div key={key} className="space-y-1">
                      ...
                      <Input
                        ...
                        className="bg-card border-border/50 h-8 text-sm"
                      />
                    </div>
                  ))}
                </div>
